...
Original file is located at
    https://colab.research.google.com/drive/1g4GipDpTZdGqWAPUS1PRs6DZjklkhcdc
"""
...
class StockPricePredictor:
    ...
    def load_data(self):
        ...
        TICKER = self.ticker
        pathH = f'/content/TSLA-history.csv'
#Open historyFile in order to clean dates in preparation for a merge
        ...
        historyFile.close()
        df_ticker = pd.DataFrame(data=historyData)
        df_ticker.columns = df_ticker.iloc[0]
        df_ticker = df_ticker.drop(labels=[0],axis=0)
        df_ticker['Dates'] = df_ticker['Date'].str.split(' ').str[0]
        new_cols = ["Dates","Open","High","Low", "Close", "Volume", "Dividends", "Stock Splits"]
        df_ticker = df_ticker.reindex(columns=new_cols)
        df_ticker['Dates'] = pd.to_datetime(df_ticker["Dates"], format="%Y/%m/%d")
